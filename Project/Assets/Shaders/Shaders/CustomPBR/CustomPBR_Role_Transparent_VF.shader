Shader "ZJY/Role/CustomPBR_Role_Transparent_VF" 
{
	Properties
	{
		...
		_EnvReflectionIntensity("Enviroment Reflection Intensity",Range(0.01,4))=1
		...
		_FresnelStrength("Fresnel Strenth",Range(0,50)) = 0
		_FresnelPower("Fresnel Power", Range(0, 16)) = 1
	...
	SubShader 
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				finalColor.a = albedo.a* _TransparentIntensity;	
				return finalColor;
			};
			ENDCG
		}
		...
		Pass 
		{
				...
				UnityLight light,UnityIndirect gi)
			{
				...
				//light.color=GammaToLinearSpace(lightColor*lightIntensity);
				...
				half fresnelPower = pow(1 + dot(-viewDir, normal), _FresnelPower);
				half3 color =   (diffColor + specularTerm * specColor) * light.color * nl
                    + gi.diffuse * diffColor
                    + surfaceReduction * gi.specular * FresnelLerpFast (specColor, grazingTerm, nv)+fresnelPower*_FresnelColor*_FresnelStrength;
			...
			fixed4 frag_surf (v2f_surf IN) : SV_Target 
			{
				...
				c.a=o.Alpha* _TransparentIntensity;
